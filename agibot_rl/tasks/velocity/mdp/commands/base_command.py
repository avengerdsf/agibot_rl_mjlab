...
class UniformVelocityCommand(CommandTerm):
  ...
  def __init__(self, cfg: UniformVelocityCommandCfg, env: ManagerBasedRlEnv):
    ...
    self.metrics["command_vel_yaw"] = torch.zeros(self.num_envs, device=self.device)
    ...
    self.metrics["actual_vel_yaw"] = torch.zeros(self.num_envs, device=self.device)
    ...
    # Set by set_gamepad_source() for real hardware gamepad override in play mode.
    self._gamepad_enabled: bool = False
    self._gamepad_reader = None  # GamepadReader | None
  ...
  def _update_metrics(self) -> None:
    ...
    self.metrics["command_vel_yaw"] += self.vel_command_b[:, 2] / max_command_step
    ...
    self.metrics["actual_vel_yaw"] += self.robot.data.root_link_ang_vel_b[:, 2] / max_command_step
  ...
  def set_gamepad_source(
    self,
    reader,
    get_env_idx: Callable[[], int],
    enabled: bool = True,
  ) -> None:
    """Attach a GamepadReader as a velocity command source for play mode.

    Priority: GUI joystick > real gamepad > default resampling.
    Call with enabled=False to detach without clearing the reader reference.
    """
  ...
  def compute(self, dt: float) -> None:
    ...
    elif self._gamepad_enabled and self._gamepad_reader is not None:
      assert self._gamepad_get_env_idx is not None
      idx = self._gamepad_get_env_idx()
      cmd = self._gamepad_reader.read_command()
      self.vel_command_b[idx, 0] = cmd.lin_vel_x
      self.vel_command_b[idx, 1] = cmd.lin_vel_y
      self.vel_command_b[idx, 2] = cmd.ang_vel_z
